...
export function createPreviewClient(): CompanionClient {
  ...
  const state: CompanionSnapshot = {
    ...
    sync: { phase: 'not-connected', lastSuccess: null, message: 'Sign in to sync your files.', conflicts: 0 },
  ...
  if (['connected', 'offline', 'conflict', 'paused'].includes(scenario ?? '')) {
    ...
    state.sync = { phase: scenario === 'connected' ? 'idle' : scenario as 'offline' | 'conflict' | 'paused', lastSuccess: '2026-09-11T00:00:00Z', message: scenario === 'offline' ? 'Waiting for a connection' : scenario === 'conflict' ? 'One file needs your attention' : scenario === 'paused' ? 'Sync is paused' : 'Your files are up to date', conflicts: scenario === 'conflict' ? 1 : 0 };
  }
  return { simulated: true, async request(request) {
    ...
    if (request.action === 'sign-out') { state.syncScopes = []; state.selectedSyncScope = undefined; state.account = { status: 'signed-out', label: null }; state.sync.phase = 'not-connected'; state.sync.message = 'Sign in to sync your files.'; }
    if (request.action === 'load-sync-scopes') state.syncScopes = [{ id: 'personal', label: 'My personal work' }, { id: 'cmp_example', label: 'My team' }];
    if (request.action === 'select-sync-scope') { state.selectedSyncScope = request.scopeId; state.sync = { phase: 'paused', message: 'Ready when you are', lastSuccess: null, conflicts: 0 }; }
    ...
    if (request.action === 'resume-sync') { state.sync.phase = 'idle'; state.sync.message = 'Your files are up to date'; state.sync.lastSuccess = new Date().toISOString(); }
