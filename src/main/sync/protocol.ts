...
export function reduceSync(state: SyncState, event: Record<string, unknown>): SyncState {
  switch (event.type) {
    case 'plan':
    case 'progress':
      return { ...state, phase: 'syncing', message: 'Keeping your files up to date' };
    case 'transient-network':
      return { ...state, phase: 'offline', message: 'Waiting for a connection' };
    case 'auth-error':
      return { ...state, phase: 'not-connected', message: 'Sign in again to continue syncing' };
    case 'setup-needed':
      return { ...state, phase: 'error', message: 'Your account needs another moment. Try again shortly.' };
    case 'error':
      return { ...state, phase: 'error', message: 'Some files could not sync. Please try again.' };
    case 'conflict': {
    ...
    case 'all-complete': {
      ...
      if (event.errors.length || event.partial) {
        return { ...state, phase: 'error', message: 'Some files could not sync. Please try again.', conflicts: 0, conflictPaths: [] };
      }
      if (event.transient.length) {
        return { ...state, phase: 'offline', message: 'Waiting for a connection', conflicts: 0, conflictPaths: [] };
      }
