...
export class SecureTokenStore implements TokenStore {
  ...
  private get file() { return join(this.directory, 'account.encrypted'); }
  private serialize<T>(action: () => Promise<T>): Promise<T> {
    const result = this.queue.then(action);
    this.queue = result.catch((error: unknown) => { console.error('Account storage operation failed', error instanceof Error ? error.name : 'unknown'); });
    return result;
  }
  read(): Promise<AccountTokens | undefined> {
    return this.serialize(async () => {
      ...
      try {
        if ((await stat(this.file)).size > 128 * 1024) throw new Error('The saved account could not be read. Please sign in again.');
  ...
  clear(): Promise<void> { return this.serialize(async () => { await rm(this.file, { force: true }); await rm(`${this.file}.tmp`, { force: true }); }); }
}
