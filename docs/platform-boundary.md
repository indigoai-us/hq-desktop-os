...
## Live sync controls and conflict recovery (US-013 Linux slice)

The Sync screen (`src/renderer/screens/sync.tsx`) and status view (`src/renderer/components/sync-status.tsx`) render runner-derived state: initial reconciliation (`plan` with no pending files), pending changes (`plan` counts), transferring (`progress`), connected realtime (clean `all-complete` under `--event-push`), fallback polling (stderr `event-push watcher degraded` → `transport: polling`), offline, paused, **Last confirmed sync** (`lastSuccess` only after protocol success — never from account connectivity alone), and actionable errors. Pause / resume / retry and company scope controls are validated companion actions with pending feedback; `select-sync-scope` and `resume-sync` re-check memberships so revoked access cannot broaden sync. Conflict recovery (`src/renderer/components/conflict-list.tsx`) exposes engine choices keep / publish-local / overwrite / abort with interactive default `--on-conflict abort`; overwrite is never a sticky bulk default. Fixture evidence: `tests/e2e/sync.spec.ts`, `__tests__/stories/US-013.test.ts`. Authenticated two-client conflict convergence stays deferred. Windows / WSL2 sync-controls acceptance remains deferred.

## Redacted diagnostics and recovery guidance (US-016 Linux slice)

Settings (`src/renderer/screens/settings.tsx`) shows app/runtime versions, workspace environment, recent sync outcomes, and actionable health checks without rendering tokens, absolute roots, or file contents. Support export uses `src/main/diagnostics.ts`: an allowlisted report is redacted, previewed in-app (`preview-diagnostics`), then saved only after an explicit save dialog — never uploaded or turned into an automatic support message. Synthetic secret and path markers are stripped from both preview and file. Managed-runtime failures are distinguishable from a missing install; Retry re-runs local health probes, and Repair (`repair-runtime`) restores the owned `userData/toolchain` only while fingerprinting workspace roots to prove content is unchanged. Destructive or unsupported repairs never run automatically. Evidence: `tests/native/diagnostics.test.ts`, `__tests__/stories/US-016.test.ts`, preview scenarios `runtime-missing` / `runtime-failed`. Windows / WSL2 diagnostics acceptance remains deferred. US-022 / US-023 live support heartbeats stay separate from this manual export.

## Tray lifecycle and opt-in start at login (US-017 Linux slice)

Background operation is opt-in. Closing the main window hides to a discoverable status tray (`src/main/tray.ts` `TrayController`) only when **Keep HQ running** is enabled and a tray is available; the tray menu exposes Open HQ, Pause sync, and Quit HQ. Explicit Quit (tray or app menu) sets the quitting flag and runs `ShutdownGate` so owned sync watchers stop — hide-to-tray never replaces Quit. Start at login (`src/main/autostart.ts`) uses per-user facilities only: Linux XDG `~/.config/autostart/hq-desktop-os.desktop` (AppImage-aware Exec, idempotent write/remove, no admin) or Electron `setLoginItemSettings` on other hosts. Unpackaged builds and WSL environments refuse enablement with visible guidance so startup cannot loop invisibly; disable still clears owned entries. Linux without a tray stays **window-required** (`src/main/lifecycle.ts`): Settings disables close-to-tray, the window remains accessible, and `window-all-closed` quits. Evidence: `tests/native/background.test.ts`, `__tests__/stories/US-017.test.ts`, preview scenarios `no-tray` / `wsl-startup`. Windows login-item / tray acceptance and full WSL host startup remain deferred.


## Shared client-health contract and heartbeat reporting (US-022 Linux slice)

The companion owns a public MIT-safe health adapter under `src/main/health/`. Client attribution is **`hq-desktop-os`** (`x-hq-client-name`) — never `hq-desktop-app` or `hq-sync`. Install identity and monotonic sequence persist under app `userData/client-health/{native|wsl2}/` so native and WSL clients cannot race each other or the CLI `~/.hq` writers. The reporter builds validated heartbeats with component versions, real sync state, attempt vs genuine completed success, consecutive failures, conflicts, updater state, and a separate engine watermark. Bounded local-file facts (`local-files.ts`) expose only closed metadata with a 15-minute cache — no paths, content, or tokens. A scheduler emits on startup, every five minutes, and on debounced health changes. Production HTTP POST to `/v1/client-health/heartbeat` stays **off** unless `HQ_CLIENT_HEALTH_REPORTING=1` and a signed-in bearer are both present; sign-out clears the token. Evidence: `tests/native/client-health.test.ts`, `__tests__/stories/US-022.test.ts`, `docs/client-health.md`. Live support-view proof on Windows 11 / Ubuntu 24.04 / WSL2 and upstream version-floor mapping remain deferred; `prd.passes` stays false until that evidence exists.
