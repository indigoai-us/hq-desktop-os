...
describe('desktop navigation boundary', () => {
  ...
  it.each(['https://example.com', 'http://127.0.0.1:4174', 'http://127.0.0.1.evil.test:4173', 'javascript:alert(1)', 'data:text/html,hello', 'about:blank', 'invalid'])('rejects foreign destinations: %s', (url) => {
  ...
  it('prevents packaged navigation to other files or remote hosts', () => {
    ...
    expect(isAllowedNavigation('about:blank', 'file:///app/index.html')).toBe(false);
  });
});
